import { ChevronRight, Paperclip, Plus, SquareDashed } from "lucide-react";
    ...
    <div className="flex flex-col gap-4 h-full">
      ...
      <ScrollArea className="flex-1 h-full">
        <div className="flex flex-col gap-2 p-2">
          {selectedObjective ? (
            selectedObjective.activities?.length && selectedObjective.activities?.length > 0 ? (
              selectedObjective.activities?.map((activity) => (
                <CardItem
                  key={activity.id}
                  title={activity.name}
