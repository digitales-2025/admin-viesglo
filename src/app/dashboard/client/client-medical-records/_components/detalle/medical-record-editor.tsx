...
} from "@/app/dashboard/admin/medical-records/_hooks/useMedicalRecords";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function MedicalRecordDetails({ recordId, mode }: MedicalRecordDetailsProps) {
  ...
  if (!data) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-center p-4">
        ...
        <Button variant="outline" onClick={() => router.push("/dashboard/client/medical-records")} className="mt-4">
  ...
  if (!isEditing && (!data.diagnosticValues || data.diagnosticValues.length === 0)) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-center p-4">
        ...
        <Button onClick={() => router.push(`/dashboard/client/medical-records/${recordId}/edit`)} className="mt-4">
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      setTimeout(() => {
        window.location.href = `/dashboard/client/medical-records/${recordId}/details`;
      }, 1000);
