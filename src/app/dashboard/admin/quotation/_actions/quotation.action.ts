...
/**
 * Descarga cotizaciones en formato XLS
 */
export async function downloadQuotationsXls(filters?: QuotationFilters): Promise<Blob | null> {
  try {
    // Construir los parámetros de consulta a partir de los filtros
    const queryParams = new URLSearchParams();
    if (filters) {
    ...
    if (err !== null || !response) {
      console.error("Error al descargar cotizaciones", err);
      return null;
    }
