...
import { downloadPaymentsXls } from "../_actions/payment.action";
...
export function DownloadExcelButton({ filters }: DownloadExcelButtonProps) {
  ...
  const handleDownload = async () => {
    try {
      setIsLoading(true);
      const blob = await downloadPaymentsXls(filters);
      ...
      link.href = url;
      const date = new Date();
      link.setAttribute("download", `pagos-${date.toLocaleDateString()}.xlsx`);
