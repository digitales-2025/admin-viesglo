                    ...
                    <HoverCardContent className="w-80">
                      <div className="space-y-2">
                        ...
                        <div className="text-sm text-muted-foreground space-y-1">
                          ...
                          <div>
                            <span className="font-medium">Location:</span> {getItemPath(item, allItems)}
                          </div>
                          <div>
                            <span className="font-medium">Items:</span> {item.children?.length || 0} direct items
                          </div>
                        </div>
                      </div>
                    </HoverCardContent>
...
export default function TreeView({
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent) => {
      ...
      const items = Array.from(dragRef.current.querySelectorAll("[data-tree-item]")) as HTMLElement[];

      const startY = dragStart;
      const currentY = e.clientY;
      const [selectionStart, selectionEnd] = [Math.min(startY || 0, currentY), Math.max(startY || 0, currentY)];
  ...
  return (
    <div className="flex gap-4">
      <div ref={treeRef} className="bg-background p-6 rounded-xl border w-full space-y-4 min-w-[600px] relative">
            ...
            >
              ...
              {showExpandAll && (
                <div className="flex gap-2 shrink-0">
                  <Button type="button" variant="ghost" size="sm" className="h-10 px-2" onClick={handleExpandAll}>
                    <ChevronDown className="h-4 w-4 mr-1" />
                    Expandir todo
                  </Button>
                  <Button type="button" variant="ghost" size="sm" className="h-10 px-2" onClick={handleCollapseAll}>
